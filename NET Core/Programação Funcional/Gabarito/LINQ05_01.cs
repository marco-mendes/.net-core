    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var queryNestedGroups =
                from pdt in produtos
                group pdt by pdt.CategoriaId into CatGroup1
                from AnoGroup in
                    (from pdt in CatGroup1
                    group pdt by pdt.Validade.Year)
                group AnoGroup by CatGroup1.Key;

                foreach (var outerGroup in queryNestedGroups)
    {
        Console.WriteLine($"Produtos de Categoria = {outerGroup.Key}");
        foreach (var innerGroup in outerGroup)
        {
            Console.WriteLine($"\tAgrupados por ano: {innerGroup.Key}");
            foreach (var innerGroupElement in innerGroup)
            {
                Console.WriteLine($"\t\t{innerGroupElement.Nome} {innerGroupElement.CategoriaId}");
            }
        }
    }
        }
